...
public interface LearnItemRepository extends JpaRepository<LearnItem,Integer> {
	...
	@Query("select distinct i from Student s join s.learnItemLists l join l.learnItems i, StudentItemRecord r where s.id = :studentId and l.id in:listIds and r.student = s order by r.priority")
	...
	@Query("select i from Student s join s.learnItemLists l join l.learnItems i where s.id = :studentId and l.id in :listIds and not exists (select r from StudentItemRecord r where r.student = s and r.learnItem.id in :listIds)")
